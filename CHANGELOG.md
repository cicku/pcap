...
- `Error::LibraryNotFound`, returned on Windows when `wpcap.dll` cannot be found. It comes from
  the entry points that do not already hold a capture handle: `Device::list`, `Device::lookup`,
  `Capture::from_device`, `Capture::from_file`, `Capture::dead`, `SendQueue::new` and the
  `Linktype` name lookups.

### Changed

- Rust Edition is now `2024`.
- Windows binaries no longer import `wpcap.dll`. It is loaded on first use instead, so a binary
  built against this crate starts on a machine without Npcap and can tell its user what to
  install rather than being refused by the loader. `wpcap.lib` is no longer needed to build, and
  a default Npcap installation now works: the library is looked for in `%SYSTEMROOT%\System32\Npcap`
  as well as along the usual search path, so WinPcap compatible mode is no longer required.
- MSRV is now `1.85`.
